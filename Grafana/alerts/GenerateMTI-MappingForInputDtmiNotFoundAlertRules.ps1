...
$AlertManifest = Get-ChildItem -Path $PSScriptRoot\AlertManifests -Filter "MTI-MappingForInputDtmiNotFound.yaml"
foreach ($AlertManifest in $AlertManifest) {
    ...
    $ManifestAlertRulesFolder = ("$PSScriptRoot\AlertRules\Single Tenant\5mFrequency\{0}" -f $AlertManifestContent.Title)
    ...
    foreach ($Resource in $Resources) {
        ...
        $template = Get-Content "$PSScriptRoot\AlertTemplates\MTI-MappingForInputDtmiNotFound.json" | ConvertFrom-Json
