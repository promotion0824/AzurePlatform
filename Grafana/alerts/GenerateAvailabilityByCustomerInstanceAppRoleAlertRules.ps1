...
$AlertManifest = Get-ChildItem -Path $PSScriptRoot\AlertManifests -Filter "Availability by Customer Instance App Role.yaml"
foreach ($AlertManifest in $AlertManifest) {
    ...
    foreach ($Resource in $Resources) {
        ...
        $template = Get-Content "$PSScriptRoot\AlertTemplates\Availability by Customer Instance App Role Template.json" | ConvertFrom-Json
        ...
        $template.data[1].model.azureMonitor.region = $Location
        $template.data[1].model.azureMonitor.resources[0].region = $Location
        $template.data[1].model.azureMonitor.resources[0].resourceGroup = $Rg
        $template.data[1].model.azureMonitor.resources[0].resourceName = $resourceName
        $template.data[1].model.azureMonitor.resources[0].subscription = $subscription
        $template.data[1].model.subscription = $subscription
        $template.data[7].model.conditions[0].evaluator.params[0] = $AlertManifestContent.threshold
