...
$AlertManifest = Get-ChildItem -Path $PSScriptRoot\AlertManifests -Filter "Health Checks by Customer Instance.yaml"
foreach ($AlertManifest in $AlertManifest) {
    ...
    $Resources = (az graph query -q "Resources | project name, type, location, resourceGroup, id, tags | where type=~ '$($AlertManifestContent.ResourceType)' and name !contains 'sandbox' and name !contains 'audit' and name in ('ain-prd-eus2')" --management-groups WillowTwinDev WillowTwinPrd | ConvertFrom-Json).data
    ...
    foreach ($Resource in $Resources) {
        ...
        $template = Get-Content "$PSScriptRoot\AlertTemplates\Health Checks by Customer Instance.json" | ConvertFrom-Json
