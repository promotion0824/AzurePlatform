...
if ($WhatIf) {
    ...
    $result = Exec { az deployment mg create --management-group-id $managementGroup --location $location --template-file $templateFile --parameters $parametersFile --name $deploymentName }
    Write-Verbose $($result -join "`r`n")
}
